...
function auctioninc_shippingcalc_product_meta_box_callback( $post ) {
	...
	$lot_size = get_post_meta($post->ID, 'jigoshop_shippingcalc_prod_lot_size', true);
	...
	echo '<tr>';
	echo '<th>';
	echo '<label for="jigoshop_shippingcalc_prod_lot_size">' . __('Lot Size', 'auctioninc_shippingcalc') . '</label>';
	echo '</th>';
	echo '<td>';
	echo '<input type="text" name="jigoshop_shippingcalc_prod_lot_size" id="jigoshop_shippingcalc_prod_lot_size" value="' . esc_attr($lot_size) . '" placeholder="1">';
	echo '<p class="description">' . __('Number of separate boxes required to ship this item.  When setting lot size > 1, set the weight/dims to the average across the boxes and set Package flag to "Separately".', 'auctioninc_shippingcalc') . '</p>';
	echo '</td>';
	echo '</tr>';	
	...
	$ondemand_codes = array(
			...
			'USPPP' => __('USPS Retail Ground', 'auctioninc_shippingcalc'),
...
function auctioninc_shippingcalc_prod_save_meta_box_data( $post_id ) {
	...
	*/
	...
	$lot_size = sanitize_text_field($_POST['jigoshop_shippingcalc_prod_lot_size']);
	update_post_meta($post_id, 'jigoshop_shippingcalc_prod_lot_size', $lot_size);
